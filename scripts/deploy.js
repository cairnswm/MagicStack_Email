...
async function deploy() {
  ...
  try {
    ...
    // create a .restart file in dist with the deploy timestamp (used to trigger restarts)
    try {
      const restartFile = path.join(repoRoot, 'dist', '.restart');
      fs.writeFileSync(restartFile, deployedAt + '\n', { encoding: 'utf8' });
      console.log(`Wrote restart file: ${restartFile}`);
    } catch (err) {
      console.warn('Failed to write restart file:', err.message || err);
    }
